...
Suggestion: (one sentence)
    ...
    suspend fun detectClothingCategory(apiKey: String, imageBase64: String): com.trymeon.app.domain.model.ClothingCategory = withContext(Dispatchers.IO) {
        val contents = listOf(
        ...
        when {
            raw.contains("outerwear", true) || raw.contains("jacket", true) || raw.contains("coat", true) -> com.trymeon.app.domain.model.ClothingCategory.OUTERWEAR
            raw.contains("bottom", true) || raw.contains("pants", true) || raw.contains("skirt", true) || raw.contains("trouser", true) -> com.trymeon.app.domain.model.ClothingCategory.PANTS
            raw.contains("dress", true) -> com.trymeon.app.domain.model.ClothingCategory.DRESS
            raw.contains("shoes", true) || raw.contains("sneaker", true) || raw.contains("boot", true) -> com.trymeon.app.domain.model.ClothingCategory.SHOES
            raw.contains("bag", true) || raw.contains("purse", true) || raw.contains("handbag", true) -> com.trymeon.app.domain.model.ClothingCategory.BAG
            raw.contains("accessory", true) || raw.contains("accessories", true) -> com.trymeon.app.domain.model.ClothingCategory.ACCESSORY
    ...
    suspend fun getCompleteLookRecommendations(
        apiKey: String,
        pinnedItems: Map<com.trymeon.app.domain.model.ClothingCategory, ClothingItem?>,
        missingCategories: List<com.trymeon.app.domain.model.ClothingCategory>,
        profile: UserProfile?,
        priceHint: String = ""
    ): String = withContext(Dispatchers.IO) {
        val profileDesc = listOf(
            if (profile != null && profile.gender.isNotBlank()) "User gender: ${profile.gender}." else "",
            com.trymeon.app.domain.sourcing.BodyHints.describe(profile),
            priceHint
...
Each searchQuery must describe the garment concretely (cut, fabric, colour), never a brand — it will be searched on a Chinese marketplace.
...
"category" must be one of: $categoryNames
    ...
    suspend fun tryOnWardrobePlan(
        ...
        val thin = com.trymeon.app.domain.model.ClothingCategory.entries
...
CAT:bag|reason (under 10 words)|$gw [specific style + item]
    ...
    suspend fun completeTheLook(
        ...
        val styleLine = if (styleKeywords.isNotEmpty()) "Style they chose: ${styleKeywords.joinToString(", ")}." else ""
...
Pick 2-3 pieces that would COMPLETE a head-to-toe outfit around this anchor. Match the wardrobe's existing palette and style so the piece will actually get worn. Each query must describe the garment concretely (cut, fabric, colour) — never a brand — because it will be searched on a Chinese marketplace. If the anchor is a top, recommend bottoms / shoes / outerwear; if it's a bottom, recommend tops / shoes / outerwear. Each piece must be a different category from the anchor and from each other.
...
GAP|...
    ...
    suspend fun sourcingQuery(
        ...
        gender: String = "",
        categoryHint: String = "",
        /** Seller words for the shopper's price band, e.g. "平价 基础款" or "高端 真皮". */
        priceHint: String = ""
    ): String = withContext(Dispatchers.IO) {
        val genderHint = if (gender.isNotBlank()) "Shopper gender: $gender." else ""
        val catHint = if (categoryHint.isNotBlank()) "Likely category: $categoryHint." else ""
        val priceLine = if (priceHint.isNotBlank())
            "Price band vocabulary sellers use for this shopper: $priceHint — fold one of these words into the first two phrases so results land in that band." else ""
        val prompt = """
...
Provide exactly 6 BUY suggestions. Each should be high-impact for this specific wardrobe — not generic. Use precise descriptive queries (color, fabric, silhouette).
    ...
    suspend fun generateOutfitBoard(
        apiKey: String,
        scene: com.trymeon.app.domain.model.OutfitScene,
        suggestion: String,
        profile: UserProfile?,
